...
{
  StringNode *new_top = arena_alloc(arena, sizeof (StringNode));
  new_top->data = data;
  new_top->next = stack->count > 0 ? stack->top : NULL;
...
String str_stack_pop(StringStack *stack, Arena *arena)
{
  ...
  StringNode *next = stack->top->next;
  String data = stack->top->data;
  arena_free(arena, sizeof (StringNode));
  stack->top = next;
  stack->count--;
